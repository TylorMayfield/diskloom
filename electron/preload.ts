...
contextBridge.exposeInMainWorld('diskloom', {
  ...
  getChildren: (scanId: string, path: string, offset?: number, limit?: number) => ipcRenderer.invoke('get-children', scanId, path, offset, limit),
  getReclaimItem: (scanId: string, path: string) => ipcRenderer.invoke('get-reclaim-item', scanId, path),
  trashReclaim: (items: unknown) => ipcRenderer.invoke('trash-reclaim', items),
