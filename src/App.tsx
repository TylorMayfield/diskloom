  ...
  return <Tooltip.Provider delayDuration={300}>
    <div className="app-shell">
      ...
      : view === 'map' ? <main className="workspace">
          <section className="visual-panel"><Sunburst root={chartRoot ?? scan.root} selected={selected ?? scan.root} onSelect={inspectNode} onRequestTrash={setPendingTrash} formatSize={formatSize}/><div className="legend"><span><i className="dot folder-dot"/>Folders</span><span><i className="dot file-dot"/>Files</span><span title={`Discovery ${(scan.timings.discoveryMs / 1000).toFixed(2)}s · SQLite ${(scan.timings.persistenceMs / 1000).toFixed(2)}s · Index ${(scan.timings.indexingMs / 1000).toFixed(2)}s · Finalization ${(scan.timings.finalizationMs / 1000).toFixed(2)}s`}>{scan.itemCount.toLocaleString()} items · {(scan.durationMs / 1000).toFixed(1)}s · {scan.workerCount} workers</span></div></section>
